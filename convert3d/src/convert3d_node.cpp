    ...
    Convert3D() : Node("convert3d_node")
    {
        ...
        */
        odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "odom",10,std::bind(&Convert3D::odomCallback,this, std::placeholders::_1));
        scan_sub_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
            "/raw_scan", 10, std::bind(&Convert3D::scanCallback, this, std::placeholders::_1));
        encoder_sub_ = this->create_subscription<std_msgs::msg::Float32>(
            "/angle_topic", 10, std::bind(&Convert3D::encoderCallback, this, std::placeholders::_1));
        cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/map_3d", 10);
        current_x_ =0;//꼭 지울것!
        current_y_ =0;
        current_z_ = 1.2;
    }
...
private:
    ...
    void scanCallback(const sensor_msgs::msg::LaserScan::SharedPtr scan)
    {
        ...
        for (float i = min_rad; i <= max_rad && count < scan->ranges.size(); i += scan->angle_increment*5, count +=5)
        {
            pcl::PointXYZ point;
            ...
            std::cout<< "add new point" << count << std::endl;
        }
